...
export function syncEventActorFactory(eventId: string) {
  return (ctx: EventMachineContext, _evt: any) => (send: any, _onReceive: any) => {
    const docPath = projectConfig.events.firestoreEventDoc(eventId)
    const unsubscribeFn = firebase.firestore().doc(docPath).onSnapshot(
...
export async function timeVote(ctx: EventMachineContext, evt: { time: number } | any) {
  ...
  const times: number[] = currentVoteSnapshot.exists ? [...currentVoteSnapshot.data()!.times, evt.time] : [evt.time]
...
export async function timeUnvote(ctx: EventMachineContext, evt: { time: number } | any) {
  ...
  const displayName = firebase.auth().currentUser!.displayName || 'Unknown'
  const currentVoteSnapshot = await docRef.get()
  const times: number[] = currentVoteSnapshot.exists ? [...currentVoteSnapshot.data()!.times.filter((time: any) => time != evt.time)] : []
  const votes: EventTimeVotes = {
    displayName,
    times,
  }
